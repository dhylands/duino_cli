...
from typing import Callable, Iterable, List, Sequence, TextIO, Union, Optional
...
class ArgFinder(CompletionFinder):
    ...
    def get_completions(
        ...
        cword_prequote, cword_prefix, cword_suffix, comp_words, last_wordbreak_pos = split_line(line, cursor_posn)
        start = 1
...
class CliCompleter(Completer):
    ...
    def __init__(self, commands: List[str], create_argparser: Callable[[str], CommandArgumentParser]) -> None:
        super().__init__()
    ...
    def get_completions(self, document, complete_event: CompleteEvent) -> Iterable[Completion]:
        ...
        completions = finder.get_completions(parser, document.text_before_cursor, document.cursor_position)
